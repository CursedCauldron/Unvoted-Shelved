...
public class GlowberryStrollTask extends Behavior<GlareEntity> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel p_22538_, GlareEntity entity) {
        return !entity.isInWaterOrBubble() && entity.getBrain().getMemory(USMemoryModules.GLOWBERRIES_GIVEN.get()).get() >= 1;
    }

    @Override
    protected boolean canStillUse(ServerLevel p_22545_, GlareEntity glare, long p_22547_) {
        return (glare.getBrain().getMemory(USMemoryModules.GLOWBERRIES_GIVEN.get()).get() >= 1) && (this.darkPos != null);
    }
                ...
                for (int z = getRandomNumber(0, -range); z <= getRandomNumber(0, range); z++) {
                    for (int y = getRandomNumber(0, -range); y <= getRandomNumber(0, range); y++) {
                        ...
                        if ((level.isInWorldBounds(blockPos2) && isValidSpawnPos(groundPos, level) && level.isEmptyBlock(spacePos) && level.isEmptyBlock(blockPos2) && (level.getBlockState(blockPos2).isPathfindable(level, blockPos2, PathComputationType.LAND)) &&
                            ...
                            glare.getBrain().setMemory(USMemoryModules.DARK_POS.get(), blockPos2);
